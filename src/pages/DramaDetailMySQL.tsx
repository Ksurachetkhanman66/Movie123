...
const DramaDetail = () => {
  ...
  const fetchDramaDetails = async () => {
    ...
    try {
      ...
      // Fetch drama details
      const dramaResponse = await dramasApi.getById(id);
      if (!dramaResponse.success) throw new Error(dramaResponse.error);
      
      if (dramaResponse.data) {
        setDrama(dramaResponse.data);
      }

      // Fetch episodes
      const episodesResponse = await dramasApi.getEpisodes(id);
      if (!episodesResponse.success) throw new Error(episodesResponse.error);

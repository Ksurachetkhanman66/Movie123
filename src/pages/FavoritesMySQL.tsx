...
import { favoritesApi, Favorite, Drama } from '@/services/api';
...
const Favorites = () => {
  ...
  const removeFavorite = async (dramaId: string) => {
    try {
      const response = await favoritesApi.remove(dramaId);

      if (!response.success) throw new Error('Failed to remove favorite');
